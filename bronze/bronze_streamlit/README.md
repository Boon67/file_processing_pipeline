...
1. **Maintenance Windows**: Suspend all tasks before performing maintenance
   ```
   Click "⏸️ Suspend All Tasks" → Perform maintenance → Click "▶️ Resume All Tasks"
   ```

2. **Immediate Processing**: Upload files and process them immediately
   ```
   Upload files → Click "🔄 Execute Discovery Now" → Files process within seconds
   ```

3. **Troubleshooting**: Investigate task failures
   ```
   View task history → Check error messages → Fix issue → Resume task
   ```

4. **Cost Control**: Pause processing during low-priority periods
   ```
   Suspend tasks overnight or weekends → Resume during business hours
   ```
